...
def input_event_paths(date, depth):
    ...
    return [f"/user/andrew_0/data/events/date={(dt-datetime.timedelta(days=x)).strftime('%Y-%m-%d')}" for x in range(depth)]
 
def reaction_tag_tops(date, depth, spark):
		reaction_paths = input_event_paths(date, depth)
		reactions = spark.read\
    .option("basePath", "/user/andrew0/data/events")\
    .parquet(*reaction_paths)\
    .where("event_type='reaction'")
 
		all_message_tags = spark.read.parquet("/user/andrew0/data/events")\
